...
map.put("ordered_by",null);
map.put("limit",null);	
arrList.add( idx, map );
session.setAttribute("varexist", arrList);
}
if (objexist==false) {
	response.sendRedirect("descriptor_menu.jsp?act=add&code="+temp);				//object berhasil ditambahkan.
} else {
	response.sendRedirect("descriptor_menu.jsp?act=add&strExst=true&code="+temp);		//object udah ada, kasih warning ke user.
}
%>
