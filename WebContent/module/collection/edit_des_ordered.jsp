...
<form name="frmForm" method="post" action="actdescriptor_menu_ordered.jsp?idn=<%=item.get("id")%>&act=<% out.println(act); %>&code=<%= temp %>" >
  <table bgcolor="black" border="0" cellpadding="3" cellspacing="1" width="100%">
    ...
    <tr bgcolor="#ffffff" valign="top"> 
      ...
      <td width="67%">
     <%
      if ( sEdt==null || sEdt.equalsIgnoreCase("False"))
	{
      %> 
    ...
    <tr bgcolor="#ffffff" valign="top"> 
      ...
      if ( sEdt==null || sEdt.equalsIgnoreCase("False"))
	{
      %> 		<option value="<%=item.get("ordered_by") %>"><%=item.get("ordered_by") %></option>
			<option value="<%=item.get("ordered_by") %>">-----------------------------</option>
	<% } %>		
			<% mySQL = "SHOW FIELDS FROM "+item.get("descriptor_name");
      ...
      <td width="67%">
         <input type="text" maxlength="4" name="limit" size="2" value="<%=item.get("limit") %>">
      </td>
    </tr>
