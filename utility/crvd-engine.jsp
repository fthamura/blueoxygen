...
<jsp:useBean id="stringUtils" scope="page" class="org.blueoxygen.util.StringUtils" />
...
// Variables
int iCount=0; 
String myRecord="", mySQLAction="", name;
mySQL="";
boolean dummy=true;
String chk_upload="";
String lst_upload="";
...
if (sAction.equals("new")) {
	...
	name=(String)parameterEnum.nextElement();
	   ...
	   if (!name.equals("submit") && !name.equals("action") ) {
		...
		if (!name.equals("table")) {
			...
			if (!dummy) {
				mySQL  = mySQL + ","+name; 
				myRecord = myRecord + ","+"'"+stringUtils.replaceHTMLInput(myUpload.getRequest().getParameter(name))+"'";
			} else {
				mySQL  = mySQL + name; 
				myRecord = myRecord + "'"+stringUtils.replaceHTMLInput(myUpload.getRequest().getParameter(name))+"'";
				dummy=false;
		 ...
		 mySQLAction="INSERT INTO "+stringUtils.replaceHTMLInput(myUpload.getRequest().getParameter(name)); 
		}
	   }
	}
	...
	mySQL = mySQLAction + "(site_id, create_by," + mySQL+") VALUES ('"+session.getAttribute("GS_USER")+"','"+loginBean.getMemberId() +"',"+ myRecord + ")";
		...
		out.println("<BR><font color=\"red\">Updating....</font>");
 		...
 		while (parameterEnum.hasMoreElements()) {
			...
			if (!name.equals("submit") && !name.equals("action") && !name.equals("id") && !name.substring(0,4).equals("chk_") && !name.substring(0,4).equals("lst_") ) { 
				...
				if (!name.equals("table")) {
					if (!dummy) {
						mySQL  = mySQL + ","+name+ "='"+stringUtils.replaceHTMLInput(myUpload.getRequest().getParameter(name))+"'";
					} else {
						mySQL  = mySQL + name+"='"+stringUtils.replaceHTMLInput(myUpload.getRequest().getParameter(name))+"'";
						dummy=false;
					}
...
<BR><BR>
<b>Notes:</b><BR>
CRVD = Create, Replace, View and Delete 
