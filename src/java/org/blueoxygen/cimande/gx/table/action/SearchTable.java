...
public class SearchTable extends TableForm implements HibernateSessionFactoryAware {
	...
	public String execute() {
		...
		Criteria crit = sess.createCriteria(DBTable.class);
		if (!table.getName().equalsIgnoreCase("")){
			crit.add(Expression.like("name", "%"+table.getName()+"%"));
		}
		if (!table.getDescription().equalsIgnoreCase("")){
			crit.add(Expression.like("description", "%"+table.getDescription()+"%"));
		}
		if (!table.getTableName().equalsIgnoreCase("")){
			crit.add(Expression.like("table_name", "%"+table.getTableName()+"%"));
		}
		...
		tables = (ArrayList<DBTable>)crit.addOrder(Order.asc(orderBy))
