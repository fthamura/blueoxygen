...
public class SaveRoleSiteDescriptorACL extends RoleSiteDescriptorACLForm {
	...
	public String execute() {
		...
		if (getDescriptor().getId() != null	&& !"".equalsIgnoreCase(getDescriptor().getId())) {
			setDescriptor((Descriptor) manager.getById(Descriptor.class,getDescriptor().getId()));
		}
		getRsDescriptor().setDescriptor(getDescriptor());
		getRsDescriptor().setRoleSite(getRoleSite());
		getRsDescriptor().setLogInformation(new LogInformation(getCurrentUser().getId(), 1));
